...
import { createOrder, listOrdersForOwner, updateOrderStatus, getOrder, deleteOrder, getOrderByNumber, exportOrdersCSV, purgeOrdersByDate } from '../controllers/orderController.js';

const router = Router();
router.post('/', createOrder); // public create order
router.get('/owner', auth(['owner']), listOrdersForOwner);
router.get('/owner/export', auth(['owner']), exportOrdersCSV);
router.post('/owner/purge', auth(['owner']), purgeOrdersByDate);
